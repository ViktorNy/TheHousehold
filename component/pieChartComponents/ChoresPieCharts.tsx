...
export default function ChoresPieCharts({ currentHouseholdChores, memberList }: Props) {
    ...
    for (const chore of currentHouseholdChores) {
        ...
        if (renderData.length > 0) {
            graphicDataForChoresList.subChores = [...graphicDataForChoresList.subChores, { choreName: chore.name, renderDataListChore: renderData }];
        } else {
    ...
    for (const member of memberList) {
        ...
        if (memberAvatar) {
            graphicDataForChoresList.renderDataAllChores = [
                ...graphicDataForChoresList.renderDataAllChores,
                { value: totalCount, key: memberAvatar.avatar, svg: { fill: memberAvatar.backgroundColor } }
            ];
        }
    }
